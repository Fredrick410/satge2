...
session_start();
        ...
        $pdo = $bdd->prepare('SELECT * FROM comptable WHERE id =:num');
        $pdo->bindValue(':num', $_SESSION['id_comptable']);
        $pdo->execute();
        $entreprise = $pdo->fetch();

        $pdo = $bdd->prepare('UPDATE comptable SET new_user="Activé" WHERE id=:num');  
        $pdo->bindValue(':num', $_SESSION['id_comptable']);
        $pdo->execute();
