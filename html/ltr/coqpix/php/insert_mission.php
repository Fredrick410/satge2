...
if (isset($_POST['name_mission'])) {
    if (!isset($_POST['id_mission'])) {
        ...
        try {
            $pdo = $bdd->prepare('SELECT * FROM mission ORDER BY id DESC LIMIT 1');
            $pdo->execute();
            $mission = $pdo->fetch();
        } catch (Exception $e) {
            $response_array['status'] = 'error';
        ...
        if (!isset($mission) or empty($mission)) {
            $response_array['status'] = 'error';
            $response_array['message'] = "Mission introuvable";
            echo json_encode($response_array);
            exit();
        } else {
            try {
