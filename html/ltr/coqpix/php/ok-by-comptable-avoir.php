...
require_once 'verif_session_connect_admin.php';
    ...
    if($stock['send_files'] == "nonvalide"){
        ...
        $updat = $bdd->prepare('UPDATE stockage_admin SET send_files = ? WHERE id = ?');
        $updat->execute(array(
    ...
    if($stock['send_files'] == "valide"){
        ...
        $update = $bdd->prepare('UPDATE stockage_admin SET send_files = ? WHERE id = ?');
        ...
        $upda = $bdd->prepare('UPDATE stockage_admin SET num_saisie = ? WHERE id = ?');
        $upda->execute(array(
